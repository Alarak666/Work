...
using ERPGE.Blazor.Server.Services;
...
using ERPGE.WebApi.JWT;

namespace ERPGE.Blazor.Server {
    public class Startup {
        ...
        public void ConfigureServices(IServiceCollection services) {
            ...
            services.AddXaf<ERPGEBlazorApplication>(Configuration);
            ...
            services.AddXafSecurity(options => {
                ...
                options.UserType = typeof(ERPGE.Module.BusinessObjects.ApplicationUser);
                // ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
                // If you use PermissionPolicyUser or a custom user type, comment out the following line:
                options.UserLoginInfoType = typeof(ERPGE.Module.BusinessObjects.ApplicationUserLoginInfo);
                ...
                options.Events.OnAuthenticated = (externalAuthenticationContext) => {
                    ...
                    externalAuthenticationContext.Principal.Identity.AuthenticationType != SecurityDefaults.WindowsAuthentication && !(externalAuthenticationContext.Principal is WindowsPrincipal)) {
                        ...
                        object CreateApplicationUser(string userName, string providerUserId) {
                            if(objectSpace.FirstOrDefault<ERPGE.Module.BusinessObjects.ApplicationUser>(user => user.UserName == userName) != null) {
                                throw new ArgumentException($"The username ('{userName}') was already registered within the system");
                            }
                            var user = objectSpace.CreateObject<ERPGE.Module.BusinessObjects.ApplicationUser>();
                        ...
                        ISecurityUserLoginInfo FindUserLoginInfo(string loginProviderName, string providerUserId) {
                            return objectSpace.FirstOrDefault<ERPGE.Module.BusinessObjects.ApplicationUserLoginInfo>(userLoginInfo =>
            ...
            services.AddSwaggerGen(c => {
                ...
                c.SwaggerDoc("v1", new OpenApiInfo {
                    Title = "ERPGE API",
                    Version = "v1",
                    Description = @"Use AddXafWebApi(options) in the ERPGE.Blazor.Server\Startup.cs file to make Business Objects available in the Web API."
                });
                    ...
                    Flows = new OpenApiOAuthFlows() {
                        AuthorizationCode = new OpenApiOAuthFlow() {
                            ...
                            Scopes = new Dictionary<string, string> {
                                ...
                                { @"[Enter the scope name in the ERPGE.Blazor.Server\Startup.cs file]", @"[Enter the scope description in the ERPGE.Blazor.Server\Startup.cs file]" }
                            }
                        }
                    }
                });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
            if(env.IsDevelopment()) {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => {
                    c.SwaggerEndpoint("/swagger/v1/swagger.json", "ERPGE WebApi v1");
